...
# r_part.c particle system lives in quake/particles.py (shared with the demo
# client). Re-export its tables/constants so the names below resolve here and
# the pinned particle tests can import PT_*/_RAMP*/MAX_PARTICLES from quake.sv.
from .particles import (                                          # noqa: F401
    _RAMP1, _RAMP2, _RAMP3, PT_STATIC, PT_GRAV, PT_SLOWGRAV, PT_FIRE,
    PT_EXPLODE, PT_EXPLODE2, PT_BLOB, PT_BLOB2, MAX_PARTICLES, _BIG_BURST)
...
# particle tables/types/limits (r_part.c) are defined in quake/particles.py and
# re-imported at the top of this module (PT_*, _RAMP*, MAX_PARTICLES, _BIG_BURST).
...
class Server:
    ...
    # The particle effects below delegate to quake/particles.py (shared with the
    # demo client). The server passes its current time; the bodies are identical.
    def _cap_particles(self):
        particles.cap(self.particles)
    ...
    def _rocket_trail(self, start, end, ttype):
        # Delegate to quake/particles.py. _tracercount is id's static counter
        # that alternates the tracer kick across calls; bridge it through a
        # one-element holder so the attribute (read/reset by the tests) stays
        # the single source of truth.
        holder = [self._tracercount]
        particles.rocket_trail(self.particles, start, end, ttype, self.time,
                               holder)
        self._tracercount = holder[0]

    def _advance_particles(self, dt):
        self.particles = particles.advance(self.particles, self.time, dt,
